(function(){
  ...
  function render(d){if(!d)return;
    ...
    if(d.metrics){const ns=document.querySelectorAll('#s3-numbers .n');[d.metrics.institutions,d.metrics.hours,d.metrics.learners].forEach((v,i)=>{if(ns[i]){ns[i].textContent=v||'—';ns[i].classList.toggle('is-text',String(v||'').trim().length>7)}});}
